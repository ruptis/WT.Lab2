...
<div class="card mb-3">
    <div class="card-header d-flex gap-4 align-items-start">
        <h3 class="card-title"><a href="<c:url value="/topic?id=${topic.id}"/>" class="text-decoration-none"><c:out value="${topic.name}"/></a></h3>
    ...
    <div class="card-footer text-muted d-flex gap-4 align-items-end">
        ...
        <div class="d-flex ms-auto gap-2 align-items-end">
            ...
            <a href="<c:url value="/profile?id=${topic.author.id}"/>" class="link-danger text-decoration-none lead" style="line-height: 1.2;"><c:out value="${topic.author.username}"/></a>
        </div>
    </div>
</div>